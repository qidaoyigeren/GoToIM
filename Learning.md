...
核心技术栈是 Go + gRPC + Kafka + Redis + TCP/WebSocket。项目最大的亮点是**双通道消息自适应投递**——不是简单的"直推失败丢给 MQ"，而是有完整的状态追踪、幂等去重、ACK 确认、离线队列清理这条闭环。
...
4. ACK 状态机：Pending → Delivered → Acked/Failed，每个状态变迁都有持久化
