...
func (b *Builder) Build(target resolver.Target, cc resolver.ClientConn, opts resolver.BuildOptions) (resolver.Resolver, error) {
	...
	dsn := strings.SplitN(endpoint, "?", 2)
	if len(dsn) == 0 {
		return nil, fmt.Errorf("grpc resolver: parse target.Endpoint(%s) failed! the endpoint is empty", endpoint)
	}
	// parse params info
	zone := os.Getenv("ZONE")
...
func (r *Resolver) newAddress(instances []*naming.Instance) {
	...
	for n, ins := range instances {
		...
		if rpcAddr == "" {
			log.Printf("grpc resolver: invalid rpc address(%s,%s,%v) found!", ins.AppID, ins.Hostname, ins.Addrs)
			continue
		}
		if weight <= 0 {
			if totalWeight == 0 {
