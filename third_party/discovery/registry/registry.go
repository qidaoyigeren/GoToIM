...
func (r *Registry) Polls(arg *model.ArgPolls) (ch chan map[string]*model.InstanceInfo, new bool, err error) {
	...
	for i := range arg.AppID {
		...
		if err == errors.NothingFound {
			log.Printf("Polls zone(%s) env(%s) appid(%s) error(%v)", arg.Zone, arg.Env, arg.AppID[i], err)
			return
		}
		if err == nil {
			ins[arg.AppID[i]] = in
	...
	for i := range arg.AppID {
		...
		if !ok {
			...
			log.Printf("Polls from(%s) new connection(%d)", arg.Hostname, connection.count)
		} else {
			connection.count++ // NOTE: there maybe have more than one connection on the same hostname!!!
			if ch == nil {
				ch = connection.ch
			}
			log.Printf("Polls from(%s) reuse connection(%d)", arg.Hostname, connection.count)
		}
	...
	for _, conn := range conns {
		...
		for i := 0; i < conn.count; i++ {
			...
			case conn.ch <- map[string]*model.InstanceInfo{appid: ii}: // NOTE: if chan is full, means no poller.
				log.Printf("broadcast to(%s) success(%d)", conn.arg.Hostname, i+1)
			case <-time.After(time.Millisecond * 500):
				log.Printf("broadcast to(%s) failed(%d) maybe chan full", conn.arg.Hostname, i+1)
			}
		}
	}
}
...
func (r *Registry) DelConns(arg *model.ArgPolls) {
	...
	for i := range arg.AppID {
		...
		conns, ok := r.conns[k]
		if !ok {
			log.Printf("DelConn key(%s) not found", k)
			continue
		}
		if connection, ok := conns[arg.Hostname]; ok {
			if connection.count > 1 {
				log.Printf("DelConns from(%s) count decr(%d)", arg.Hostname, connection.count)
				connection.count--
			} else {
				log.Printf("DelConns from(%s) delete(%d)", arg.Hostname, connection.count)
