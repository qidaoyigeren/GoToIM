...
func (n *Node) Register(c context.Context, i *model.Instance) (err error) {
	...
	if err != nil {
		log.Printf("node be called(%s) register instance(%v) error(%v)", n.registerURL, i, err)
	}
	return
}
...
func (n *Node) Cancel(c context.Context, i *model.Instance) (err error) {
	...
	if err != nil {
		log.Printf("node be called(%s) instance(%v) already canceled", n.cancelURL, i)
	}
	return
}
...
func (n *Node) Renew(c context.Context, i *model.Instance) (err error) {
	...
	if err == errors.ServerErr {
		log.Printf("node be called(%s) instance(%v) error(%v)", n.renewURL, i, err)
...
func (n *Node) call(c context.Context, action model.Action, i *model.Instance, uri string, data interface{}) (err error) {
	...
	if err = n.client.Post(c, uri, "", params, &res); err != nil {
		log.Printf("node be called(%s) instance(%v) error(%v)", uri, i, err)
		return
	}
	if res.Code != 0 {
		log.Printf("node be called(%s) instance(%v) response code(%v)", uri, i, res.Code)
