...
```mermaid
flowchart LR
  Producer["业务系统 / Notify Server / HTTP Push API"]

  subgraph L1["Logic 层"]
    Logic["Logic\nHTTP/gRPC API\nSession / Online / Discovery"]
  end

  subgraph L2["Router 层"]
    Router["Message Router\nRouteByUser / RouteByRoom / RouteBroadcast\nIdempotency / ACK / State / Attempt"]
  end

  subgraph Infra["可靠通道"]
    Redis["Redis\nSession / Offline Queue / Message State"]
    Kafka["Kafka\nUser / Room / Broadcast / ACK / DLQ Topics"]
  end

  subgraph L3["Worker 层"]
    Worker["Delivery Worker\nConsumer Group / Retry / DLQ / Room Aggregation"]
  end

  subgraph L4["Comet 层"]
    Comet["Comet Gateway\nTCP / WebSocket / gRPC\nBucket / Channel / Room / PriorityQueue"]
  end
  ...
  Producer --> Logic
  Logic --> Router
  Router -->|"Fast Path: gRPC direct push"| Comet
  Router -->|"Slow Path: offline store"| Redis
  Router -->|"Slow Path: enqueue"| Kafka
  Kafka --> Worker
  Worker -->|"gRPC fan-out"| Comet
  Comet <--> Client
  Client -->|"ACK"| Comet
  Comet --> Logic
  Logic --> Router
...
```mermaid
sequenceDiagram
  participant B as Business / Notify
  ...
  B->>L: POST /goim/push/mids
  L->>R: RouteByUser(msg_id, uid, op, body)
  R->>R: HSETNX 抢占 msg_id, 记录 pending
  R->>S: 查询 Session / 在线状态
  alt 用户在线
    R->>C: gRPC PushMsg, 500ms/session
    C->>U: TCP / WebSocket OpRaw
    R->>S: MarkDelivered
  else 离线或直推失败
    R->>S: ZADD offline:{uid}
    R->>K: Enqueue user message
    K->>W: Consumer Group
    W->>C: gRPC PushMsg / Broadcast
    C->>U: TCP / WebSocket OpRaw
  end
  U->>C: OpPushMsgAck
  C->>L: Receive / ACK
  L->>R: HandleACK
  R->>S: delivered -> acked, remove offline
  R->>K: Publish ACK event
```
...
- Policy 配置：优先级、TTL、ACK 策略、最大重试、DLQ 策略、通知模板。
...
│   ├── mq/                      # MQ 抽象、消息 envelope、ACK/DLQ 类型
│   ├── notify/                  # 订单通知业务平台
│   ├── grpcx/                   # gRPC 拦截器
...
| Notify Policy | `configs/notify_policy.yaml` |
...
- [Notify Server 后端改进记录](./docs/order-notification-backend-improvements.md)
- [订单通知平台设计](./docs/superpowers/specs/2026-05-16-order-notify-platform-design.md)
- [MySQL-only 存储设计](./docs/superpowers/specs/2026-05-19-remove-sqlite-mysql-only-design.md)
