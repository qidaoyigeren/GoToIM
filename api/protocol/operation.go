...
const (
	...
	// OpSendMsgAck server -> client: message send confirmation with msg_id
	OpSendMsgAck = int32(18)
	// OpPushMsgAck client -> server: push message received confirmation
	OpPushMsgAck = int32(19)
	// OpSyncReq client -> server: sync offline messages request
	OpSyncReq = int32(20)
	// OpSyncReply server -> client: sync offline messages reply
	OpSyncReply = int32(21)

	// OpKickConnection server -> comet: kick a connection
