...
func newCometClient(addr string) (*grpc.ClientConn, comet.CometClient, error) {
...
func NewComet(in *naming.Instance, c *conf.Comet) (*Comet, error) {
	...
	var err error
	if cmt.grpcConn, cmt.client, err = newCometClient(grpcAddr); err != nil {
		return nil, err
	}
...
func (c *Comet) Push(arg *comet.PushMsgReq) (err error) {
	...
	default:
		err = fmt.Errorf("push chan full server:%s", c.serverID)
		log.Errorf("c.Push chan full serverId:%s", c.serverID)
	}
	return
}
...
func (c *Comet) BroadcastRoom(arg *comet.BroadcastRoomReq) (err error) {
	...
	default:
		err = fmt.Errorf("room chan full server:%s", c.serverID)
		log.Errorf("c.BroadcastRoom chan full serverId:%s", c.serverID)
	}
	return
}
...
func (c *Comet) Broadcast(arg *comet.BroadcastReq) (err error) {
	...
	default:
		err = fmt.Errorf("broadcast chan full server:%s", c.serverID)
		log.Errorf("c.Broadcast chan full serverId:%s", c.serverID)
	}
	return
}
...
func (c *Comet) process(pushChan chan *comet.PushMsgReq, roomChan chan *comet.BroadcastRoomReq, broadcastChan chan *comet.BroadcastReq) {
	for {
		...
		case broadcastArg := <-broadcastChan:
			_, err := c.client.Broadcast(c.ctx, &comet.BroadcastReq{
		...
		case roomArg := <-roomChan:
			_, err := c.client.BroadcastRoom(c.ctx, &comet.BroadcastRoomReq{
		...
		case pushArg := <-pushChan:
			_, err := c.client.PushMsg(c.ctx, &comet.PushMsgReq{
				Keys:    pushArg.Keys,
				Proto:   pushArg.Proto,
