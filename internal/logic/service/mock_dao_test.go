...
type mockRetryDAO struct {
	mu      sync.Mutex
	entries []retryEntry
	counts  map[string]int // msg_id -> retry count
}

func newMockRetryDAO() *mockRetryDAO {
	return &mockRetryDAO{
		counts: make(map[string]int),
	}
}

func (m *mockRetryDAO) EnqueueRetry(_ context.Context, msgID string, uid int64, score float64) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.entries = append(m.entries, retryEntry{msgID: msgID, uid: uid, score: score})
	return nil
}

func (m *mockRetryDAO) DequeueRetry(_ context.Context, minScore, maxScore float64, limit int) ([]string, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	var result []string
	var remaining []retryEntry
	for _, e := range m.entries {
		if e.score >= minScore && e.score <= maxScore && len(result) < limit {
			result = append(result, fmt.Sprintf("%s:%d", e.msgID, e.uid))
		} else {
...
func (m *mockRetryDAO) IncrRetryCount(_ context.Context, msgID string) (int, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.counts[msgID]++
	return m.counts[msgID], nil
}

func (m *mockRetryDAO) RemoveRetry(_ context.Context, msgID string, uid int64) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	for i, e := range m.entries {
		if e.msgID == msgID && e.uid == uid {
